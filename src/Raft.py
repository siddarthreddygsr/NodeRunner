...
class RaftNode:
    def __init__(self, ip, port,  peers, election_timeout_min, election_timeout_max, heartbeat_interval):
        ...
        self.vote_count = 0
        self.vote_list = [0]*(len(self.peers) - 1)
    ...
    def request_votes(self):
        ...
        self.vote_count += 1
        self.vote_list[int(self.port)%5000] = 'ls'
        for peer in self.peers:
            if peer == self.port:
                continue
            message = {
                "src_port": self.port,
                "candidateId":self.port,
                "dst_port": peer,
                "type": "request_vote",
    ...
    def handle_request_vote_response(self, message):
        ...
        print("_______________start ______________")
        # print(message['term'])
        ...
        # if candidate_id not in self.voted_for:
            # return
        print(message)
        if message["vote_granted"]:
            print("_______________before______________")
            print("vote_granted")
            self.vote_list[int(message['src_port'])%5000] = 1
            # num_votes = sum([1 for peer in self.peers if self.match_index[peer] >= self.commit_index and peer != self.port])
            self.vote_count += 1
            print(self.vote_count)
            if self.vote_count > len(self.peers) // 2:
                self.leader_port = self.port
                print(self.leader_port)
                print(self.vote_count)
                self.state = "leader"
                print(f"{self.port} is the leader")
                print(f"Voted members = {self.vote_list}")
                self.vote_count = 0
                self.vote_list = [0]*len(self.peers)
                self.cancel_election_timer()
                self.start_heartbeat_timer()
                self.send_append_entries()
                # while True:
    ...
    def ping(self,port):
        tn = telnetlib.Telnet()
        try:
            tn.open("127.0.0.1", self.port, timeout=2)
